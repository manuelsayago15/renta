                              ...
                              },
                              {
                                  ...
                                  render: function(data, type, row) {
                                    var oficina = {
                                      ...
                                      return `<a class="disabledUser text-success" style="cursor:pointer" onclick='activar_oficina(`+oficina+`)'>Reactivar</a>`;
...
function activar_oficina(oficina){
    alert(oficina.nombre)
    const userMssg = document.querySelector('.user-message');
    var dominio = 'http://10.156.160.21:8000/api/'; //direccion de la api para consumir
    ...
    //const disabledUserName = e.target.parentElement.parentElement.children[0].innerHTML;
    Swal.fire({
        title: `¿Está seguro que desea activar a `+id+`?`,
        showCancelButton: true,
        confirmButtonColor: '#f53030',
        confirmButtonText: 'Si, quiero'
    }).then((result) => {
        if (result.value) {
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    oficinas_inactivas();
                    if(resp.actualizado >0){
                        userMssg.innerHTML = `Se reactivó a <span class="font-weight-bold">`+id+`</span>`;
                        userMssg.classList.remove('d-none');
                        setTimeout(() => { userMssg.classList.add('d-none') }, 3000);   
                    }            
                },
...
function inactivar_oficina(id){
    ...
    //const activeUserName = e.target.parentElement.parentElement.children[0].innerHTML;
            Swal.fire({
                title: `¿Está seguro que quiere desactivar a `+id+`?`,
                showCancelButton: true,
                confirmButtonColor: '#f53030',
                confirmButtonText: 'Si, quiero'
            }).then((result) => {
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function(resp) {
                            ...
                            oficinas_activas();
                            if(resp.actualizado >0){
                                userMssg.innerHTML = `Se desactivó a <span class="font-weight-bold">`+id+`</span>`;
                                userMssg.classList.remove('d-none');
                                setTimeout(() => { userMssg.classList.add('d-none') }, 3000);
                                
                            }  
                            else {
                                 userMssg.innerHTML = `Se desactivó a <span class="font-weight-bold">`+resp.error+`</span>`;
                                 userMssg.classList.remove('d-none');
                                 setTimeout(() => { userMssg.classList.add('d-none') }, 5000);
                            }         
                        },
                        error: function(resp) {
