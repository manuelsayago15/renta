jQuery(document).ready(function ($) {
    ...
    $(function() {//ventas por categoria del dashboard imprimir monedas
      ...
      var num='*';
      $.ajax({
          url: dominio + 'polizaventamonedasgtesucursal/' + id_oficina+'/'+num,
          dataType: 'json',
          type: 'GET',
          data: {
    ...
    $(function() {//ventas por top ejecutivos del dashboard imprimir monedas
      ...
      $.ajax({
          ...
          success: function(resp) {
              console.log(resp)
              if(resp.data.moneda==0){
                removeSpinner($('#topEjecutivos')) 
                $('#topEjecutivos').append('<p id="topEjecutivosp">No hay datos que mostrar.<p>'); 
              }
              var arreglo = Array(0);
              for (var i = 0; i < resp.data.length; i++) {
               arreglo[i] = resp.data[i];
              ...
              $("#topEjecutivos-tabs").html("");
              $("#topEjecutivos-tabs").append(`<li class="nav-item"><a class="nav-link nav-link--dashboard active" style="cursor:pointer" onclick="mostrar_top_ejecutivos_graficas_barras(` + resp.data[0].moneda + `)" href="#ventas-categorias-todos" data-toggle="tab" aria-controls="ventas-categorias-todos" aria-selected="true" id="topEjecutivos-uf">`+resp.data[0].unidad+`</a></li>`);
              for (var j = 1; j < resp.data.length; j++) {
             
                  $("#topEjecutivos-tabs").append(`<li class="nav-item"><a class="nav-link nav-link--dashboard" style="cursor:pointer" onclick="mostrar_top_ejecutivos_graficas_barras(` + resp.data[j].moneda + `)" href="#ventas-categorias-todos" data-toggle="tab" aria-controls="ventas-categorias-todos" aria-selected="true" id="topEjecutivos-uf">`+resp.data[j].unidad+`</a></li>`);
     
             }
             mostrar_top_ejecutivos_graficas_barras(resp.data[0].moneda);
          },
          error: function(resp) {
              console.log(resp)
          }
  ...
  function mostrar_categorias_graficas_barras(m){
      ...
      var num='*';
      $.ajax({
          url: dominio + 'polizaventasgtesucursal/' + id_oficina+'/'+num,
          dataType: 'json',
          type: 'GET',
          data: {
          ...
          success: function(resp) {
              ...
              $('#ventas-categorias-todos').append('<canvas id="ventas-categorias-chart"><canvas>'); 
  ...
  function mostrar_15dias_graficas_ondas(m){
    ...
    var num='15';
    $.ajax({
        url: dominio + 'polizaultimasventasgtesucursal/' + id_oficina+'/'+num,
        dataType: 'json',
        type: 'GET',
        data: {
...
function  menor_mayor(m){
  ...
  var num='15';
  $.ajax({
      url: dominio + 'polizaventarangogtesucursal/' + id_oficina+'/'+num,
      dataType: 'json',
      type: 'GET',
      data: {
  ...
  function mostrar_top_ejecutivos_graficas_barras(m){
     
    $('#topEjecutivos-chart').remove(); 
    addSpinner($('#topEjecutivos'));
    ...
    var num='*';
    $.ajax({
        url: dominio + 'polizatopejecutivogtesucursal/' + id_oficina+'/'+num,
        dataType: 'json',
        type: 'GET',
        data: {
        ...
        success: function(resp) {
            //console.log(resp) 
            var ejecutivos = Array(0);
            var total = Array(0);

            for (var i=0;i<resp.data.length;i++) {
                if(resp.data[i].id_moneda==m){
                  ejecutivos.push(resp.data[i].nombres+" "+resp.data[i].apellidos);
                  total.push(resp.data[i].total);
                }
            }


            $('#topEjecutivos').append('<canvas id="topEjecutivos-chart"><canvas>'); 
        
            var ventasCategoriasUFChartZone = document.querySelector("#topEjecutivos-chart").getContext('2d');
...
function mostrar_top_corredores_graficas_barras(m){
  ...
  var num='*';
  $.ajax({
      url: dominio + 'polizatopintermediariogtesucursal/' + id_oficina+'/'+num,
      dataType: 'json',
      type: 'GET',
      data: {
